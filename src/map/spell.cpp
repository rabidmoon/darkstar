    ...
    bool CanUseSpell(CBattleEntity* PCaster, CSpell* spell)
    {
        ...
        if (spell != nullptr)
        {
            ...
            if (PCaster->objtype == TYPE_MOB || 
                (PCaster->objtype == TYPE_PET && static_cast<CPetEntity*>(PCaster)->getPetType() == PETTYPE_AUTOMATON) ||
                (PCaster->objtype == TYPE_PET && static_cast<CPetEntity*>(PCaster)->getPetType() == PETTYPE_TRUST))
            {
