...
CAIPetDummy::CAIPetDummy(CPetEntity* PPet)
{
	...
	m_najelithRangedRecast = 20000;  // Ranged Attack every 11 seconds
	m_najelithSharpshotRecast = 300000;
	m_najelithBarrageRecast = 300000;
	
	m_rugVokeRecast = 30000;
	m_rugMagicRecast = 4000;
	m_rugFlashRecast = 45000;
	m_rugHealRecast = 20000;
	m_magicGadalarRecast = 4000;
	m_gadalarEleRecast = 20000;
...
void CAIPetDummy::ActionAbilityStart()
{
	...
	if (m_PPet->m_PetID == PETID_ZEID && m_PPet->health.tp >= 1000 && m_PBattleTarget != nullptr){  
		...
		else if (zeidSCPartner == nullptr)
		{
			...
			if (lvl > 71) {
				uint8 wsrandom = dsprand::GetRandomNumber(1, 3); // Ground Strike or Spinning Slash
				for (int i = 0; i < m_PPet->PetSkills.size(); i++) {
					auto PMobSkill = battleutils::GetMobSkill(m_PPet->PetSkills.at(i));
				...
				uint8 wsrandom = dsprand::GetRandomNumber(1, 3); //Sickle Moon or Crescent Moon
				for (int i = 0; i < m_PPet->PetSkills.size(); i++) {
					auto PMobSkill = battleutils::GetMobSkill(m_PPet->PetSkills.at(i));
				...
				uint8 wsrandom = dsprand::GetRandomNumber(1, 3); //Crescent Moon or Freezebite
				for (int i = 0; i < m_PPet->PetSkills.size(); i++) {
					auto PMobSkill = battleutils::GetMobSkill(m_PPet->PetSkills.at(i));
				...
				uint8 wsrandom = dsprand::GetRandomNumber(1, 3); //Freezebite or Hard Slash
				for (int i = 0; i < m_PPet->PetSkills.size(); i++) {
					auto PMobSkill = battleutils::GetMobSkill(m_PPet->PetSkills.at(i));
	...
	if (m_PPet->m_PetID == PETID_NAJELITH && m_PPet->health.tp >= 1000 && m_Tick >= m_LastNajelithWSTime + 7000 && m_PBattleTarget != nullptr){
		...
		if (lvl > 71) {
			...
			for (int i = 0; i < m_PPet->PetSkills.size(); i++) {
				...
				else if (PMobSkill->getID() == 3763 && wsrandom < 2) { //Empyreal Arrow
					mobwsID = 199;
					SetCurrentMobSkill(PMobSkill);
					SetCurrentWeaponSkill(mobwsID);
					break;
			...
			uint8 wsrandom = dsprand::GetRandomNumber(1, 2); // Blast Arrow or Sidewinder
			for (int i = 0; i < m_PPet->PetSkills.size(); i++) {
				auto PMobSkill = battleutils::GetMobSkill(m_PPet->PetSkills.at(i));
				if (PMobSkill->getID() == 3761 && wsrandom >= 2) { //Blast Arrow
					mobwsID = 197;
					SetCurrentMobSkill(PMobSkill);
					SetCurrentWeaponSkill(mobwsID);
					break;
				}
				else if (PMobSkill->getID() == 3763 && wsrandom < 2) { //Sidewinder
					mobwsID = 196;
					SetCurrentMobSkill(PMobSkill);
					SetCurrentWeaponSkill(mobwsID);
					break;
			...
			uint8 wsrandom = dsprand::GetRandomNumber(1, 2); // Sidewinder Exlusively
			for (int i = 0; i < m_PPet->PetSkills.size(); i++) {
				auto PMobSkill = battleutils::GetMobSkill(m_PPet->PetSkills.at(i));
				if (PMobSkill->getID() == 3763 && wsrandom >= 2) { //Blast Arrow
					mobwsID = 196;
					SetCurrentMobSkill(PMobSkill);
					SetCurrentWeaponSkill(mobwsID);
					break;
			...
			uint8 wsrandom = dsprand::GetRandomNumber(1, 2); // Flaming Arrow Only
			for (int i = 0; i < m_PPet->PetSkills.size(); i++) {
				auto PMobSkill = battleutils::GetMobSkill(m_PPet->PetSkills.at(i));
				if (PMobSkill->getID() == 3759 && wsrandom >= 2) { //Flaming Arrow
					mobwsID = 192;
					SetCurrentMobSkill(PMobSkill);
					SetCurrentWeaponSkill(mobwsID);
					break;
				}
				else if (PMobSkill->getID() == 3759 && wsrandom < 2) { //Flaming
					mobwsID = 192;
					SetCurrentMobSkill(PMobSkill);
					SetCurrentWeaponSkill(mobwsID);
					break;
	...
	if (m_PPet->m_PetID == PETID_RUG && m_PPet->health.tp >= 1000 && m_PBattleTarget != nullptr){
		int16 mobwsID = -1;
		if (lvl > 71) {
			uint8 wsrandom = dsprand::GetRandomNumber(1, 3); // Victory Beacon or Spinning Slash
			for (int i = 0; i < m_PPet->PetSkills.size(); i++) {
				auto PMobSkill = battleutils::GetMobSkill(m_PPet->PetSkills.at(i));
				if (PMobSkill->getID() == 1878 && wsrandom >= 2) { //Victory Beacon
					SetCurrentMobSkill(PMobSkill);
					break;
				}
				else if (PMobSkill->getID() == 3753 && wsrandom < 2) { //Spinning Slash
			...
			for (int i = 0; i < m_PPet->PetSkills.size(); i++) {
				...
				else if (PMobSkill->getID() == 3749 && wsrandom < 2) { //Power Slash
					mobwsID = 48;
					SetCurrentMobSkill(PMobSkill);
					SetCurrentWeaponSkill(mobwsID);
					break;
			...
			for (int i = 0; i < m_PPet->PetSkills.size(); i++) {
				auto PMobSkill = battleutils::GetMobSkill(m_PPet->PetSkills.at(i));
				if (PMobSkill->getID() == 3749) { //Power Slash
					mobwsID = 48;
					SetCurrentMobSkill(PMobSkill);
					SetCurrentWeaponSkill(mobwsID);
					break;
	...
	if (m_PPet->m_PetID == PETID_GADALAR && m_PPet->health.tp >= 1000 && m_PBattleTarget != nullptr){
		int16 mobwsID = -1;
		if (lvl > 71) {
			uint8 wsrandom = dsprand::GetRandomNumber(1, 3); // Salamander Flame or Spiral Hell
			for (int i = 0; i < m_PPet->PetSkills.size(); i++) {
				auto PMobSkill = battleutils::GetMobSkill(m_PPet->PetSkills.at(i));
				if (PMobSkill->getID() == 1833 && wsrandom >= 2) { //Salamander Flame
					SetCurrentMobSkill(PMobSkill);
					break;
				}
				else if (PMobSkill->getID() == 3807 && wsrandom < 2) { //Spiral Hell
					mobwsID = 104;
					SetCurrentMobSkill(PMobSkill);
					SetCurrentWeaponSkill(mobwsID);
					break;
			...
			uint8 wsrandom = dsprand::GetRandomNumber(1, 2); // Cross Reeper or Guillotine
			for (int i = 0; i < m_PPet->PetSkills.size(); i++) {
				auto PMobSkill = battleutils::GetMobSkill(m_PPet->PetSkills.at(i));
				if (PMobSkill->getID() == 3806 && wsrandom >= 2) { //Cross Reeper
					mobwsID = 103;
					SetCurrentMobSkill(PMobSkill);
					SetCurrentWeaponSkill(mobwsID);
					break;
				}
				else if (PMobSkill->getID() == 3805 && wsrandom < 2) { //Guillotine
					mobwsID = 102;
					SetCurrentMobSkill(PMobSkill);
					SetCurrentWeaponSkill(mobwsID);
					break;
			...
			uint8 wsrandom = dsprand::GetRandomNumber(1, 2); // Guillotine or Nightmare Scythe
			for (int i = 0; i < m_PPet->PetSkills.size(); i++) {
				auto PMobSkill = battleutils::GetMobSkill(m_PPet->PetSkills.at(i));
				if (PMobSkill->getID() == 3805 && wsrandom >= 2) { //Guillotine
					mobwsID = 102;
					SetCurrentMobSkill(PMobSkill);
					SetCurrentWeaponSkill(mobwsID);
					break;
				}
				else if (PMobSkill->getID() == 3804 && wsrandom < 2) { //Nightmare
					mobwsID = 99;
					SetCurrentMobSkill(PMobSkill);
					SetCurrentWeaponSkill(mobwsID);
					break;
			...
			uint8 wsrandom = dsprand::GetRandomNumber(1, 2); // Nightmare Scythe or Slice
			for (int i = 0; i < m_PPet->PetSkills.size(); i++) {
				auto PMobSkill = battleutils::GetMobSkill(m_PPet->PetSkills.at(i));
				if (PMobSkill->getID() == 3804 && wsrandom >= 2) { //Nightare Scythe
					mobwsID = 99;
					SetCurrentMobSkill(PMobSkill);
					SetCurrentWeaponSkill(mobwsID);
					break;
				}
				else if (PMobSkill->getID() == 3803 && wsrandom < 2) { //Slice
					mobwsID = 96;
					SetCurrentMobSkill(PMobSkill);
					SetCurrentWeaponSkill(mobwsID);
					break;
			...
			uint8 wsrandom = dsprand::GetRandomNumber(1, 2); // Slice Only
			for (int i = 0; i < m_PPet->PetSkills.size(); i++) {
				auto PMobSkill = battleutils::GetMobSkill(m_PPet->PetSkills.at(i));
				if (PMobSkill->getID() == 3803 && wsrandom >= 2) { //Slice
					mobwsID = 96;
					SetCurrentMobSkill(PMobSkill);
					SetCurrentWeaponSkill(mobwsID);
					break;
...
void CAIPetDummy::ActionAttack()
{
	...
	if (m_PPet->m_PetID == PETID_GADALAR)
	{
		if (m_Tick >= m_LastGadalarMagicTime + m_magicGadalarRecast) // Check Every 4 Seconds as universal check
		{
			int16 spellID = -1;
			uint16 family = m_PPet->m_Family;
			uint16 petID = m_PPet->m_PetID;
	...
	if (m_PPet->m_PetID == PETID_NAJELITH)
	{
		if (m_Tick >= m_LastNajelithRangeTime + m_najelithRangedRecast) //Ranged Attack
		{
			...
			for (int i = 0; i < m_PPet->PetSkills.size(); i++) {
				...
				if (PMobSkill->getID() == 946) { //Ranged Attack
					SetCurrentMobSkill(PMobSkill);
					ShowWarning(CL_GREEN"NAJELITH RANGED ATTACK!!!!!!\n" CL_RESET);	
					break;
				}
			}	
			preparePetAbility(m_PBattleSubTarget);
		...
		if ((m_Tick >= m_LastNajelithBerserkTime + m_najelithBerserkRecast) && trustlevel >= 50)
		{
			ShowDebug("Najelith Berserk \n");
			m_PWeaponSkill = nullptr;
			int16 mobjaID = -1;			
			for (int i = 0; i < m_PPet->PetSkills.size(); i++) {
				auto PMobSkill = battleutils::GetMobSkill(m_PPet->PetSkills.at(i));
			...
			//ShowWarning(CL_RED"Last Berserk TIme is %u! \n" CL_RESET, m_LastNajelithBerserkTime);
			//ShowWarning(CL_RED"m_Tick TIme is %u! \n" CL_RESET, m_Tick);	
			//ShowWarning(CL_RED"m_najelithBerserkRecast TIme is %u! \n" CL_RESET,m_najelithBerserkRecast);				
			return;	
		}
		else if (m_Tick >= m_LastNajelithSharpshotTime + m_najelithSharpshotRecast && trustlevel >= 5)
		{
			ShowDebug("Najelith sharpshot \n");
			m_PWeaponSkill = nullptr;
			int16 mobjaID = -1;			
			for (int i = 0; i < m_PPet->PetSkills.size(); i++) {
				auto PMobSkill = battleutils::GetMobSkill(m_PPet->PetSkills.at(i));
	...
	if (m_PPet->m_PetID == PETID_RUG)
	{
		if (m_Tick >= m_LastRugVokeTime + m_rugVokeRecast)  // Provoke is Priority over Spell Check
		{
			m_PWeaponSkill = nullptr;
			int16 mobjaID = -1;			
			for (int i = 0; i < m_PPet->PetSkills.size(); i++) {
			...
			preparePetAbility(m_PBattleSubTarget);
			m_LastRugVokeTime = m_Tick;
			return;	
		}		
		if (m_Tick >= m_LastRugMagicTime + m_rugMagicRecast) // Check Every 4 Seconds as universal check
		{
			int16 spellID = -1;
			uint16 family = m_PPet->m_Family;
			uint16 petID = m_PPet->m_PetID;
	...
	if (m_PPet->getPetType() == PETTYPE_TRUST && m_Tick >= m_LastEngageStart + 7000 + tpWait && m_PPet->m_PetID == PETID_RUG && m_PPet->health.tp >= 1000 && m_PPet->PetSkills.size() > 0)
	{
		m_PBattleSubTarget = m_PBattleTarget;
		m_ActionType = ACTION_MOBABILITY_START;
		ActionAbilityStart();
	...
	if (m_PPet->getPetType() == PETTYPE_TRUST && m_Tick >= m_LastEngageStart + 7000 + tpWait && m_PPet->m_PetID == PETID_MIHLI && m_PPet->health.tp >= 1000 && m_PPet->PetSkills.size() > 0)
	{
		m_PBattleSubTarget = m_PBattleTarget;
		m_ActionType = ACTION_MOBABILITY_START;
		ActionAbilityStart();
...
int16 CAIPetDummy::GadalarSpell()
{	
	int16 spellID = -1;
	uint8 level = m_PPet->PMaster->GetMLevel();
	if (m_Tick >= m_LastGadalarEleCast + m_gadalarEleRecast && m_ActionType != ACTION_MAGIC_CASTING)	
	{
		m_PBattleSubTarget = m_PBattleTarget;
		if (level >= 73)
		if (m_PPet->health.mp > 134)
		...
		m_LastGadalarEleCast = m_Tick;
	}			
	else 
	{
		m_gadalarEleRecast = 20000; //Gadalar
	}
	m_LastGadalarMagicTime = m_Tick;
	return spellID;	
}
...
int16 CAIPetDummy::RugSpell()
{
	...
	CBattleEntity* mostWounded = getWounded(trigger);
	CBattleEntity* sleptMage = getSleptMage();
	if (m_Tick >= m_LastRugTimeHeal + m_rugHealRecast)  // Look for last magic healing spell time 
	{
		// Cure White Mage Trust if slept
		if (sleptMage != nullptr && (sleptMage->StatusEffectContainer->HasStatusEffect(EFFECT_SLEEP) == true || sleptMage->StatusEffectContainer->HasStatusEffect(EFFECT_LULLABY) == true))
		{
		...
		else if (mostWounded != nullptr)
		{
			...
			m_rugHealRecast = 25000; 
			m_magicHealCast = 1; // 1 means casting a spell
		}
		else
		{
			m_LastRugTimeHeal = m_Tick; // reset mtick no eligible healing spell to cast
			m_rugHealRecast = 14000;		
		}
	} 
	else if (m_Tick >= m_LastRugFlash + m_rugFlashRecast)  // Look for last flash spell time 
	{
		m_PBattleSubTarget = m_PBattleTarget;
		if (level >= 37)
		if (m_PPet->health.mp > 25)  	
		...
		m_rugFlashRecast = 50000;
		m_LastRugFlash = m_Tick;
	}		
	return spellID;
}
