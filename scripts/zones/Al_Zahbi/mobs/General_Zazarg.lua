...
function onMobDeath(mob,killer)
    printf("General Zazarag has fallen");
	...
	for i, person in pairs(players) do
        person:PrintToPlayer("General Zazarag has fallen",0x1C);
	end		
    SetServerVariable("ZazargCapture",1);
	captured = captured + 1;	
	SetServerVariable("Generals_Captured",captured);
    -- Set Prisoner Status
	if (besieged == 1) then
	    SetServerVariable("Zazarag_Prison",1);
		local pris = GetNPCByID(16998978);
	    pris:setStatus(STATUS_NORMAL);
	elseif (besieged == 2) then
	    SetServerVariable("Zazarag_Prison",2);	
		local pris = GetNPCByID(17031707);
	    pris:setStatus(STATUS_NORMAL);		
	elseif (besieged == 3) then
	    SetServerVariable("Zazarag_Prison",3);
		local pris = GetNPCByID(17043985);
	    pris:setStatus(STATUS_NORMAL);		
	end
