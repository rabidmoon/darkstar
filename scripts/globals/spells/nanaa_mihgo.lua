...
function onMagicCastingCheck(caster,target,spell)
	...
	elseif (((bf:getBcnmID() > 1382) and (bf:getBcnmID() < 1387)) or ((bf:getBcnmID() > 1376) and (bf:getBcnmID() < 1382))) then
	caster:PrintToPlayer("Nanaa Mihgo is unavailable for this fight.  Please try another Trust.",0xD);	
	elseif(not caster:canUsePet()) then
		return MSGBASIC_CANT_BE_USED_IN_AREA;
	elseif (not caster:isUniqueAlly(78)) then
