...
function onMagicCastingCheck(caster,target,spell)
	...
	elseif ((bf ~= nil) and (((bf:getBcnmID() > 1384) and (bf:getBcnmID() < 1387)) or ((bf:getBcnmID() > 1376) and (bf:getBcnmID() < 1384)))) then
	    caster:PrintToPlayer("Zazarg is unavailable for this fight.  Please try another Trust.",0xD);		
	    return MSGBASIC_CANT_BE_USED_IN_AREA;
	elseif (bf == nil and (zone == 139 or zone == 144 or zone == 146 or zone == 206)) then
	  caster:PrintToPlayer("Zazarg cannot be summoned outside a BC fight.",0xD);
	  return MSGBASIC_CANT_BE_USED_IN_AREA;		
	elseif(not caster:canUsePet()) then
		return MSGBASIC_CANT_BE_USED_IN_AREA;
	elseif (not caster:isUniqueAlly(98)) then
       	caster:PrintToPlayer("Zazarg is already summoned.",0xD);
    else
	return 0;
	end
end;
...
function onSpellCast(caster,target,spell)
	caster:spawnAlly(98);
	caster:PrintToPlayer(string.format("(Zazarg) For the Empire!", caster:getName()),0xF);    
	return 0;
end;