...
local AdelheidTokens = killer:getVar("CurrentTokens_Adel");
local AdelheidPoints = killer:getVar("CurrentPoints_Adel");

local AyameTokens = killer:getVar("CurrentTokens_Ayame");
local AyamePoints = killer:getVar("CurrentPoints_Ayame");

local CurillaTokens = killer:getVar("CurrentTokens_Cur");
local CurillaPoints = killer:getVar("CurrentPoints_Cur");

local DarcullinTokens = killer:getVar("CurrentTokens_Darc");
local DarcullinPoints = killer:getVar("CurrentPoints_Darc");

local ExcenmilleTokens = killer:getVar("CurrentTokens_Excen");
local ExcenmillePoints = killer:getVar("CurrentPoints_Excen");

local KupipiTokens = killer:getVar("CurrentTokens_Kup");
local KupipiPoints = killer:getVar("CurrentPoints_Kup");
...
local trustpoint = mob:getBaseExp() * 1;
	...
	if ((not killer:isUniqueAlly(91)) or not killer:isUniqueAlly(86) or not killer:isUniqueAlly(75) or not killer:isUniqueAlly(76)
	    or not killer:isUniqueAlly(77) or not killer:isUniqueAlly(78) or not killer:isUniqueAlly(79) or not killer:isUniqueAlly(80)
		or not killer:isUniqueAlly(81) or not killer:isUniqueAlly(82) or not killer:isUniqueAlly(83) or not killer:isUniqueAlly(84) 
        or not killer:isUniqueAlly(85) or not killer:isUniqueAlly(86)) then-- Means that they are in the party.  Need to list all because it displays trust points
