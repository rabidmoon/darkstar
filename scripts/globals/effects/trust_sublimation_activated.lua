...
function onEffectTick(target,effect)
	...
	local limit = math.floor((target:getBaseHP() + target:getMod(MOD_HP)));
	...
	if (complete) then
		target:delStatusEffectSilent(EFFECT_TRUST_SUBLIMATION_ACTIVATED);
		target:addStatusEffectEx(EFFECT_TRUST_SUBLIMATION_COMPLETE,EFFECT_SUBLIMATION_COMPLETE,store,0,7200);
		printf("Sublimation Completed");
	else
