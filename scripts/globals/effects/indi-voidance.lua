...
-- Party Effect
    ...
    for i,member in ipairs(nearbyChars) do
	    if (member:getObjType() ~= TYPE_MOB and not member:hasStatusEffect(EFFECT_INDI_VOIDANCE) and member:getAllegiance() == target:getAllegiance() and geoname == plead) then
		    printf("Add effect");
		    member:addStatusEffect(EFFECT_EVASION_BOOST_II, effect:getPower(), 0, 3);
	    end
	end
